...
interface CreateCustomLayerProps {
  ...
  customLayer: ShallowRefOrNo<CustomLayerInterface | (new () => CustomLayerInterface)>;
  beforeId?: string;
}
...
export function useCreateCustomLayer({
  ...
  function createLayer() {
    ...
    if (typeof customLayer.value === 'function' && customLayer.value instanceof Object) {
  ...
  function setBeforeId(beforeIdVal?: string) {
    if (mapInstance.value && layerId.value && layer.value && hasLayer(mapInstance.value, layerId.value)) {
