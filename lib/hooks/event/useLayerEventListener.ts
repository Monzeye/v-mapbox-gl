...
import { onUnmounted, watchEffect } from 'vue-demi';
...
export function useLayerEventListener<T extends keyof MapLayerEventType>(props: LayerEventProps<T>) {
  ...
  const stopEffect = watchEffect((onCleanUp) => {
    if (mapInstance.value && layer.value) {
      const layerId = typeof layer.value === 'string' ? layer.value : layer.value.id
