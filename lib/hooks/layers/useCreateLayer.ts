import type { CreateBaseLayerActions, ILayer, Nullable, ShallowRefOrNo } from '@/types';
...
import { shallowRef, unref, computed, watch } from 'vue-demi';
...
export function useCreateLayer<Layer extends ILayer>({
	...
	function setBeforeId(beforeIdVal?: string) {
		if (mapInstance.value && layer.value && hasLayer(mapInstance.value, layerId)) {
	...
	function createLayer() {
		...
		if (map && source.value && !layer.value && !hasLayer(map, layerId)) {
			...
			if (typeof source.value === 'object') {
				// @ts-ignore
				sourceData = source.value.id ? source.value.id : source.value.type ? source.value : '';
			}
			map.addLayer(
				{
					id: layerId,
