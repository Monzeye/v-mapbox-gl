import { watch, ref, unref, shallowRef, computed, onUnmounted } from 'vue-demi'
import type { Ref } from 'vue-demi'
import type { Map, MapSourceDataEvent, VideoSource, VideoSourceOptions } from 'mapbox-gl'
...
interface CreateImageSourceActions {
  ...
  setCoordinates: (coordinates: Required<VideoSourceOptions>["coordinates"]) => void;
}
...
export function useCreateVideoSource({
  ...
  function setCoordinates(coordinatesVal: Required<VideoSourceOptions>["coordinates"]) {
  ...
  function initImageSource() {
    ...
    if (!source.value && map && !hasSource(map, sourceId) && urls.length && coordinates.value?.length) {
