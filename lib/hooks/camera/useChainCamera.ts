...
class ChainCamera {
  ...
  _lerp<T extends LerpParams>(a: T, b: T, t: number): T {
    if (Array.isArray(a) && Array.isArray(b)) {
      ...
      for (let i = 0; i < Math.min(a.length, b.length); i++) result[i] = a[i] * (1.0 - t) + b[i] * t;
      return (result as T);
    } else {
  ...
  _animate(phase: number) {
    ...
    const lngLat = this._lerp(prevItem.lngLat as [number, number], currentItem.lngLat as [number, number], this._easeInOut(phase));
    const altitude = this._lerp(prevItem.altitude, currentItem.altitude, phase);
    const lookAtLngLat = this._lerp(prevItem.lookAtLngLat as [number, number], currentItem.lookAtLngLat as [number, number], phase);
...
export function useChainCamera({
  ...
  list,
  autoplay = false
}: { map: ShallowRef<Nullable<Map>>, autoplay?: boolean; list: ChainCameraItem[] }) {
